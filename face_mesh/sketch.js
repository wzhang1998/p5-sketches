let sketch = function(p){
  ...
  // p.preload = function(){
  //   shapesData = p.loadJSON("presets/new.json", () => {
  ...
  p.draw = function(){
    ...
    p.textSize(15);
    p.fill(255);
    p.text("press 'q' to delete png",80,10)
    p.pop();
  ...
  p.faceMesh = function(){
    ...
    let minimun = p.min(dMouse);
    closest = dMouse.indexOf(minimun);
  ...
  p.putImage = function(){
   let [betweenEyesX, betweenEyesY] =
      [detections.multiFaceLandmarks[0][168].x*p.width,
      detections.multiFaceLandmarks[0][168].y*p.height];
    // test shape middle eye pos
    // p.fill(255);
    // p.rect(betweenEyesX, betweenEyesY,20,20);
    p.image(img,betweenEyesX,betweenEyesY,160,80)
